...
El tipo de contenedor ''Python3-Executor'' es para ejecutar commandos de shell en addicion a codigo de Python 3. Se extiende de los contenedores definidos para el ''Shell-Executor'' para proveer funcionalidad de ejecutar commandos en un shell. El punto de entrada de ejecucion es el archivo ''main.py'' a nivel raiz del repositorio. Tanto Debian como Alpine requieren la instalacion de Python 3, porque lastimosamente su version de Python por defecto sigue siendo 2.7 que se deberia considerar practicamente obseleta. Docker requiere que se vuelve a definir los volumenes y para la adecuada ejecuccion del codigo, tambien se define la ubicacion incial de ejecuccion como el mismo volumen que se monta desde un sistema de ficheros externo al contenedor.
...
El tipo de contenedor ''PostgreSQL-Executor'' es para ejecutar commandos de shell en addicion a SQL de PostgreSQL. Se extiende de los contenedores definidos para el ''Shell-Executor'' para proveer funcionalidad de ejecutar commandos en un shell. El archivo ''init.sql'' se ejecuta primero y por lo tanto puede ser utilizado para generar y llenar la base de datos. El archivo ''main.sql'' se ejecuta despues y por lo tanto podria ser util para ejecutar commandos SQL definidos por el usuario. Tanto Debian como Alpine requieren la instalacion de PostgreSQL, tanto cliente como servidor. Docker requiere que se vuelve a definir los volumenes y para la adecuada ejecuccion del codigo, tambien se define la ubicacion incial de ejecuccion como el mismo volumen que se monta desde un sistema de ficheros externo al contenedor.
...
WORKDIR "/code"
\end{lstlisting}
La primera y segunda linea define herencia del contenedor Alpine Shell-Executor que definimos previamente. La tercera y cuarta linea instala PostgreSQL dentro del contenedor en adicion a inicializar la base de datos. La sexta hasta decimaseptima linea levanta el motor de base de datos PostgreSQL con la finalidad de crear un usuario y base de datos por defecto sobre el cual se puede trabajar. Finalizando este proceso se desactiva el contenedor para reducir el tamaño del mismo y no introducir comportamiento desconocido. La decimonovena linea vuelve a declarar el volumen de Docker porque al aparecer la version actual de Docker a proposito no suporta herencia de esta linea en los Dockerfile. La vigesima linea define la ubicacion inicial utilizada cuando se inicia el contenedor.
...
El diseño planteado para el sistema EduNube propone dos tipos de autenticacion:
\begin{itemize}
\item Clasica para usuarios humanas
\item Basada en Criptografica Asimentrica para ofrecer mayor seguridad y permitir integracion con otros sistemas como GitEDU
\end{itemize}

\subsubsection{Autenticacion Clasica}
Para la administracion del sistema poer operador humano se considera necesario la autenticacion clasica mediante usuario y contraseña. Para la misma se ocuparon la misma implementacion del framework Django y codigo reutilizado de GitEDU que tiene el mismo proposito.

\subsubsection{Autenticacion basado en Criptografia Asimetrica}
Para ofrecer mayor seguridad al sistema de ejecucion de codigo, EduNube, se considera mas optimo utilizar una sistema de autenticacion basado en criptografia asimetrica de una manera similar las llaves de SSH, donde el servidor tiene la oportunidad de conocer bien la identidad del cliente. Aunque no se ha logrado encontrar una implementacion factible para la solucion planteada, se considera alta factibilidad en utilizar un especie de API token similar a otros sistemas. Para el mismo se logró encontrar un estandar, JSON Web Tokens que tiene la finalidad de ayudar a un servidor identificar y verificar la validez de un cliente desconfiable mediante datos que el servidor mismo cifra con un tipo de llave privada.

A continuacion se presenta la logica para decifrar y cifrar el token con el fin de generar/actualizarlo y tambien poderlo validar contra una tabla interna de la base de datos a lo que un cliente se lo envia:
\begin{lstlisting}
...
Como parte de la administracion de este aspecto del sistema de autenticacion, se encuentra implementado las funcionalidades de crear, actualizar, leer y borrar estos tokenes de autenticacion, siempre y cuando el usuario logueado tenga el permiso ''auth\_admin.manage\_tokens''. Si no se cree este permiso, como es el caso de la ambiente de desarrollo, la politica de Django es solo permitir cuentas de superusuario acceder a estas vistas.
