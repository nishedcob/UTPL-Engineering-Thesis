...
% TODO: EduNube requires a Port Change in Production, Kubernetes uses :8001 for proxy, for consistency GitServerHTTPEndpoint should be changed at the same time
    	\begin{lstlisting}
kubectl proxy
...
% TODO: GitServerHTTPEndpoint uWSGI ini
...
\begin{lstlisting}
mkdir -p /static/service/uwsgi/githttpserverendpoint
find /static -type d -exec chmod +x -c {} \;
chown -c root:git /static/service/uwsgi/githttpserverendpoint
find /static/service/uwsgi/githttpserverendpoint -type f -exec chmod -c 660 {} \;
\end{lstlisting}
...
# Production:
STATICFILES_DIRS = [
    "/static/service/uwsgi/githttpserverendpoint",
]
\end{lstlisting}
