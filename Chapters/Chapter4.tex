...
Previo a realizar el desarrollo, es necesario que se prepara el ambiente para la misma con un sistema base (Debian Stretch con el Xen Hipervisor), un servidor local de Git (Git + Gitweb + NGinX), un servicio para recibir peticiones HTTP y convertirlos en sus respetivos objetos en un repositorio de Git (Git Server HTTP Endpoint), un LMS (Moodle), un servidor de Git (GitLab CE), un servidor de Virtualizacion (Docker), plantillas de maquinas virtuales (en Docker).
...
Para gestionar versiones de codigo escrito y facilitar la integracion con sistemas externas y el sistema de virtualizacion, se levanta un servidor sencillo de Git.
...
Despues se configura NGinX para trabajar con gitweb:
\begin{lstlisting}
# NGinX Config:
server {
        listen 80;
...
Ademas se configura gitweb con la siguiente configuracion:
\begin{lstlisting}
...
Para recibir llamadas HTTP y convertirlos en sus respetivos objetos en un repositorio de Git, se creó un miniservicio que realiza esta tarea, basado en la misma app de autenticacion que utiliza EduNube:
\begin{lstlisting}
pipenv --three
...
Para la ejecución de código se considera necesario la virtualización. La ejecución puramente nativa de código daría mayor superficie de ataque al servidor físico ademas de no permitir mayor flexibilidad necesaria para sostener una gran variedad de usuarios y necesidades.
...
En los cuadros \ref{tab:hipervisor-compar-i} y \ref{tab:hipervisor-compar-ii} se presenta las características consideras y un puntaje relativo (sobre 5) dada a cada una de las mismas según las investigaciones, experiencias y opiniones del autor. A continuacion se explica el porque de las valoraciones:
\begin{description}
