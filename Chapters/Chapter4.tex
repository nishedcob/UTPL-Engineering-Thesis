...
% TODO: Fix up and replace with a figure
...
Para el manejo de datos dentro del API interna, se estandariza los objectos de datos con las siguientes clases genericas, las cuales pueden ser sobreescritas en cualquier backend con una finalidad de agregar o cambiar funcionalidad existente:
\lstset{language=Python}
\begin{lstlisting}
...
class GenericChange:
    ...
    def __str__(self):
        return "Change: (%s) :: \"%s\" :: %s :: %s" %\
            (self.repository, self.comment, self.author,\
                self.timestamp)
...
class GenericChangeFile:
    ...
    def validate_change(self, change):
        ...
        if not isinstance(change, GenericChange):
            raise ValueError("Change should be a " + \
                "Change Object")
    ...
    def __str__(self):
        return "ChangeFile: (%s) :: %s" %\
            (self.change, self.file_path)
\end{lstlisting}
\lstset{language=Bash}
...
% TODO: Gestor de Persistencia de Codigo
